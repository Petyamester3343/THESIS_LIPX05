...
{
    internal class GeneticOptimizer(Dictionary<string, Node> nodes, List<Edge> edges, int populationSize = 50, int generations = 100) : IOptimizer
    {
        private readonly Random rnd = new();
        private readonly SolverLogManager logManager = new("GeneticOptimizer");

        private readonly Dictionary<(Node, Node), double> edgeCosts = edges.ToDictionary(e => (e.From, e.To), e => e.Cost);
        private readonly Dictionary<(Node, Node), bool> dependencyCheck = edges.ToDictionary(e => (e.From, e.To), e => true);
        ...
        public List<Node> Optimize()
        {
            ...
            List<List<Node>> population = [.. Enumerable.Range(0, populationSize).Select(_ => nodes.Values.OrderBy(_ => rnd.Next()).ToList())];
            ...
            {
                // evaluate fitness with a list containing instances of an anonymous type
                ...
                {
                    List<Node> p1 = parents[rnd.Next(parents.Count)];
                    List<Node> p2 = parents[rnd.Next(parents.Count)];
        ...
        private double Evaluate(List<Node> path)
        {
            ...
            for (int i = 0; i < path.Count - 1; i++)
            {
                ...
                total += edgeCosts.TryGetValue((from, to), out double cost) ? cost : 0.0;
                penalty += dependencyCheck.ContainsKey((to, from)) ? DependencyPenalty : 0.0;
            }
